...
use super::{
    binary, builtin3, builtin4, field, scalar::NumericType, Constructible, IntoVal, Lift, Scalar,
...
pub fn vec3<T: ScalarType>(
    x: impl IntoVal<Value = Scalar<T>>,
    y: impl IntoVal<Value = Scalar<T>>,
    z: impl IntoVal<Value = Scalar<T>>,
) -> Vec3<T> {
    builtin3("vec3", x, y, z)
}
